...
const ExternshipForm = () => {
  ...
  // Update reservation when month or year changes
  useEffect(() => {
    if (selectedMonth && selectedYear) {
      setReservation(`${selectedMonth} ${selectedYear}`);
    } else {
      setReservation("");
    }
  ...
  const getMonthOptions = () => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      months.push(`${monthName} 1-15`);
      months.push(`${monthName} 16-${daysInMonth}`);
    }
    return months;
  };

  // Generate year options (7 years starting from 2026)
